...
;     selected by setup.  Any error message will automatically reset the 
...
;[RLA] $7FFF, and RAM from $8000 up.  The VT1802 firmware uses the last 2.5K
;[RLA] of RAM for the screen buffer and other storage.
FIRST	EQU	04000H		;[RLA] address (in ROM) of this code
RAM	EQU	08000H		;[RLA] "bottom" of RAM 
RAMTOP	EQU	0F500H-1	;[RLA] "top" of usable RAM
...
;[RLA] Equates for the VT1802 firmware!
VT1802  EQU	0010H		;[RLA] base address for VT1802 EPROM
VCALL	EQU	VT1802+(3* 0)	;[RLA]  0 - our standard call routine
VRETURN	EQU	VT1802+(3* 1)	;[RLA]  1 - our standard return routine
VTRAP	EQU	VT1802+(3* 2)	;[RLA]  2 - breakpoint trap
CONPUT	EQU	VT1802+(3* 3)	;[RLA]  3 - send characters to the console
CONGET	EQU	VT1802+(3* 4)	;[RLA]  4 - read keystrokes from the console
BRKTST	EQU	VT1802+(3* 5)	;[RLA]  5 - check for console BREAK 
SERPUT	EQU	VT1802+(3* 6)	;[RLA]  6 - get character from serial port
SERGET	EQU	VT1802+(3* 7)	;[RLA]  7 - send character to serial port
ISSBRK	EQU	VT1802+(3* 8)	;[RLA]  8 - test for serial framing error
TIN	EQU	VT1802+(3* 9)	;[RLA]  9 - redirect console input to PS/2 keyboard
TOUT	EQU	VT1802+(3*10)	;[RLA] 10 - redirect console output to VT display
SINP	EQU	VT1802+(3*11)	;[RLA] 11 - redirect console input to serial port
SOUT	EQU	VT1802+(3*12)	;[RLA] 12 - redirect console output to serial port
DEFCONS EQU	VT1802+(3*13)	;[RLA] 13 - restore console to default
BSAVE	EQU	VT1802+(3*14)	;[RLA] 14 - save BASIC program/data w/XMODEM
BLOAD	EQU	VT1802+(3*15)	;[RLA] 15 - load BASIC program/data w/XMODEM
BEXIT	EQU	VT1802+(3*16)	;[RLA] 16 - re-enter VT monitor
;BGMODE	EQU	VT1802+(3*17)	;[RLA] 17 - select graphics or text mode
;BPLOT	EQU	VT1802+(3*18)	;[RLA] 18 - plot a single point
;BLINE	EQU	VT1802+(3*19)	;[RLA] 19 - draw lines in graphics mode
BPLAY	EQU	VT1802+(3*20)	;[RLA] 20 - play simple music
BKEY	EQU	VT1802+(3*21)	;[RLA] 21 - test for key press
BWAIT	EQU	VT1802+(3*22)	;[RLA] 22 - delay for a number of ticks
BTIME	EQU	VT1802+(3*23)	;[RLA] 23 - return current system uptime
...
;	DB	066H, 'D', 'M', 'A', 'P', 80H+'T', 0A4H			;[RLA] was ..DMAPT
...
;	DB	065H, 'W', 'F', 'L', 80H+'N', 0AFH			;[RLA] ..WFLN
;	DB	065H, 'R', 'F', 'L', 80H+'N', 0B0H			;[RLA] ..RFLN
...
;	 ..B8 IS USED FOR \ TO CLOSE BINARY DATA
	...
	DS	028H	;[RLA] adjust for the keywords removed
...
;[RLA]   The PLAY statement plays a tune using the CDP1863 timer on the
;[RLA] VT1802 board.  The argument to PLAY is a string of characters which
...
EXTAB	DW	REM		; 080H
	...
	DW	BYE		; 0ADH
	DW	EXIT		; 0AEH
	DW	NOOP		; 0AFH was SUWBF
	DW	NOOP		; 0B0H was SURBF
	DW	RENUM		; 0B1H
	DW	EDIT		; 0B2H
	DW	FORMAT		; 0B3H
